...
import util.lr_decay_copy as lrd
import util.misc_copy as misc
from util.datasets import build_dataset
from util.pos_embed_copy import interpolate_pos_embed
from util.misc_copy import NativeScalerWithGradNormCount as NativeScaler
...
def main(args):
    ...
    path = os.path.join(path, './work_dir')
    # args.output_dir = os.path.join(path, str(args.model) + '_lineprobe' + '_' + str(args.dataset) + '_' + str(
        # args.split) + '_' + str(args.input_size)) + '_' + str(args.postfix)
    args.output_dir = os.path.join(path, str(args.model) + '_lineprobe' + '_' + str(args.dataset) + '_' + str(
        args.split) + '_' + str(args.input_size) + '_' + str(args.drop_path) + '_' + str(args.batch_size) + '_' + str(args.warmup_epochs) + '_' + str(args.blr) + '_' + str(args.layer_decay) + '_' + str(args.postfix)) 
    ...
    for i in range(args.exp_num):
        ...
        elif args.model == 'arm_large_pz16':
            model = models_mamba.__dict__[args.model](
                num_classes=args.nb_classes,
                drop_path_rate=args.drop_path,
                global_pool=args.global_pool,
        ...
        elif args.model == 'arm_small_pz16':
            model = models_mamba.__dict__[args.model](
                num_classes=args.nb_classes,
                drop_path_rate=args.drop_path,
                global_pool=args.global_pool,
    ...
    with open(os.path.join(args.output_dir, "log.txt"), mode="a", encoding="utf-8") as f:
        f.write('Last acc1 of {} model on {} dataset: {:.2f} ± {:.2f} \n'.format(args.model, args.dataset,
                                                                                 exp_record[0, -2], exp_record[0, -1]))
        f.write(
            'Max acc1 of {} model on {} dataset: {:.2f} ± {:.2f} \n'.format(args.model, args.dataset, exp_record[1, -2],
                                                                            exp_record[1, -1]))
        f.write('Average training time on {} epoch: {} ± {} \n'.format(args.epochs, str(datetime.timedelta(
            seconds=int(exp_record[2, -2]))), \
                                                                       str(datetime.timedelta(
                                                                           seconds=int(exp_record[2, -1])))))

    print('Last acc1 of {} model on {} dataset: {:.2f} ± {:.2f}'.format(args.model, args.dataset, exp_record[0, -2],
                                                                        exp_record[0, -1]))
    print('Max acc1 of {} model on {} dataset: {:.2f} ± {:.2f}'.format(args.model, args.dataset, exp_record[1, -2],
                                                                       exp_record[1, -1]))
    print('Average training time on {} epoch: {} ± {}'.format(args.epochs,
                                                              str(datetime.timedelta(seconds=int(exp_record[2, -2]))), \
