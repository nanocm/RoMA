...
class ARM(nn.Module):
    def __init__(self, 
        ...
        print("OK", if_cls_token)
        print(use_double_cls_token)
...
def arm_small_pz16(pretrained=False, **kwargs):
    model = ARM(
        patch_size=16, embed_dim=512, depth=12, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v3", if_cls_token=True, if_devide_out=True, use_middle_cls_token=True, **kwargs)
    model.default_cfg = _cfg()
    return model
...
def arm_tiny_pz16(pretrained=False, **kwargs):
    model = ARM(
        patch_size=16, embed_dim=384, depth=12, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v3", if_cls_token=True, if_devide_out=True, use_middle_cls_token=True, **kwargs)
    model.default_cfg = _cfg()
    return model
